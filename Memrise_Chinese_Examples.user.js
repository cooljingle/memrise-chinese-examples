        ...
        function enableExamples(lang) {
            var audioPlaying,
                ...
                modalHtml = $([
                    ...
                    "                   </div>",
                    "                   <hr>",
                    "                   <h3>Show examples when tested</h3>",
                    "                   <div>",
                    "                      <label>",
            ...
            function isChinese(input) {
                return typeof(input) === "string" && input.match(new RegExp(`^${chineseUnicode}+`));
            }
            ...
            function loadDOM() {
                ...
                if(isTestBox) {
                    $('.question-row').after(exampleHtml);
                } else {
            ...
            function renderExample() {
                ...
                if(isTraditional && !example.traditional) {
                    simpToTrad(example.example, function(trad) {
                        example.exampleAutolink = example.exampleAutolink.replace(new RegExp(chineseUnicode, 'g'), _ => {
                            var char = trad[0];
            ...
            function setCurrentWord(context) {
                var p = _.find(MEMRISE.garden.screens[context.learnable_id], x => x.template === "presentation"),
                    columns = [p.item, p.definition, ...p.visible_info, ...p.hidden_info],
                    wordMatch = _.find(columns, x => x.kind === "text" && isChinese(x.value));
                word = wordMatch && wordMatch.value;
                if(word) {
                    var pinyinMatch = _.find(columns, c => c.kind === "text" && c.label.toLowerCase().match(/pinyin|pronunciation/i));
            ...
            function setModalEvents(settingsObject) {
                ...
                //colours
                $('.picker button').click((e) => {
                    e.stopPropagation();
                    var picker = "<div style='display: inline-flex; position: absolute'><div id='picker' style='width:100px; height:100px'></div><div id='slide' style='width:15px; height:100px'></div></div>";
                    var pickerElm = $(picker).insertAfter(e.target);
                    var rgb = $(e.target).css('background-color').match(/\d+/g);
                    ColorPicker(
                        pickerElm.find('#slide')[0],
                        pickerElm.find('#picker')[0],
                        function(hex, hsv, rgb) {
                            var id = $(e.target).attr('id');
                            $(e.target).css('background-color', hex);
                            settingsObject.colours[id] = hex;
                            $('.' + id).css('color', hex);
                        }).setRgb({r: rgb[0], g: rgb[1], b: rgb[2]});
                    $(document).one('click', function closePicker(e) {
                        if (pickerElm.has(e.target).length) {
                            $(document).one('click', closePicker);
                        } else {
                            pickerElm.remove();
                        }
                    });
                ...
                //show on test
                $('#show-on-test').change(function() {
                    settingsObject.showOnTest = $(this).is(':checked');
